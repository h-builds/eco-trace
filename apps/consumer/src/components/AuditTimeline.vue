...
const integrityMap = computed(() => {
  const map = new Map<string, WasmIntegrityResult['status']>();
  
  if (!isReady.value) {
    props.events.forEach(e => map.set(e.id, 'PENDING'));
    return map;
  }
  ...
  for (const event of props.events) {
    ...
    if (res.status === 'VALID' && event.integrity_status === 'UNAUTHORIZED') {
       map.set(event.id, 'UNAUTHORIZED');
    } else {
       map.set(event.id, res.status);
    }
  }
...
<template>
  <div class="mt-6 flex flex-col gap-4">
    ...
    <ul role="list" class="relative border-l-2 border-surface-border pl-4 flex flex-col gap-6 ml-2 pb-2 m-0 p-0 list-none">
      <li v-for="event in events" :key="event.id" class="relative">
        <span aria-hidden="true" class="absolute -left-[23px] top-1 w-3 h-3 rounded-full bg-brand-integrity-blue ring-4 ring-surface-canvas"></span>
        ...
        <div class="bg-surface-card p-3 rounded-md shadow-subtle border border-surface-border flex flex-col gap-2 relative overflow-hidden">
          <div v-if="integrityMap.get(event.id) === 'INVALID' || integrityMap.get(event.id) === 'UNAUTHORIZED'" class="absolute left-0 top-0 bottom-0 w-1 bg-functional-alert"></div>
          <div v-else-if="integrityMap.get(event.id) === 'WARNING'" class="absolute left-0 top-0 bottom-0 w-1 bg-functional-pending"></div>
          
          <div class="flex items-center justify-between">
            <span class="text-sm font-bold text-brand-deep-charcoal">{{ event.action_type }}</span>
            <AuthenticityBadge :status="integrityMap.get(event.id) || 'PENDING'" />
          </div>
          ...
          <div class="text-xs text-functional-neutral grid grid-cols-2 gap-2 mt-1">
            <div class="flex flex-col">
              ...
              <span class="font-mono break-all line-clamp-1" :title="event.actor_id" :aria-label="`Actor ID: ${event.actor_id}`">{{ event.actor_id }}</span>
            </div>
